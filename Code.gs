...
/**
 * แปลงวันที่ Date object เป็นข้อความ พ.ศ. (dd/MM/yyyy HH:mm:ss)
 */
function _formatThaiDate(dateObj, timeFormat) {
  if (!(dateObj instanceof Date)) return _norm(dateObj);
  timeFormat = timeFormat || 'HH:mm:ss';
  const year = dateObj.getFullYear() + 543;
  const month = ('0' + (dateObj.getMonth() + 1)).slice(-2);
  const day = ('0' + dateObj.getDate()).slice(-2);
  const timeStr = Utilities.formatDate(dateObj, 'GMT+7', timeFormat);
  return day + '/' + month + '/' + year + ' ' + timeStr;
}

/**
 * กำหนดช่วงวันที่ default หากระบุช่วงวันที่ผิดพลาด
 */
function _resolveDateRange(startDateStr, endDateStr) {
  const todayStr = Utilities.formatDate(new Date(), 'GMT+7', 'yyyy-MM-dd');
  const start = _norm(startDateStr) || todayStr;
  const end = _norm(endDateStr) || todayStr;
